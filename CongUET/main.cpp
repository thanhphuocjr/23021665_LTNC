...
bool loadFont() {
    ...
    gFont_GameOver = TTF_OpenFont("fontsText/OrangeJuice/juice.ttf", 50);
...
void LoadTexture::render(int x, int y,SDL_RendererFlip flip)
{
    SDL_Rect renderQuad={x,y,realWidth,realHeight};
    SDL_RenderCopyEx(gRenderer, realTexture, NULL, &renderQuad, 0, NULL, flip);
}
...
Character::Character()
{
    ...
    real_y_position=gBackGround.GetHeight()-gCharacter.GetHeight();
    real_x_Vel=0;
    real_y_Vel=0;
}
...
void Character::move()
{
    ...
    if (real_y_position >=SCREEN_HEIGHT - gCharacter.GetHeight()) {
        real_y_position = SCREEN_HEIGHT - gCharacter.GetHeight();
        real_y_Vel = 0;
        jump=false;
    }
    ...
    if(real_x_position < 0 || (real_x_position + gCharacter.GetWidth() > SCREEN_WIDTH))
    {
        real_x_position -= real_x_Vel;
    }
}
...
{
    if(isLeft){
        gCharacter.render(real_x_position, real_y_position,SDL_FLIP_NONE);
    }
    else
    {
        gCharacter.render(real_x_position,real_y_position,SDL_FLIP_HORIZONTAL);
    }
}
...
void AddGift()
{
    int x=rand() % SCREEN_WIDTH-gGift.GetWidth();
    int y=0;
...
void AddFailScore()
{
    int x=rand() % SCREEN_WIDTH-gFail.GetWidth();
    int y=0;
...
void AddDeadline() {
        // Tạo một viên đá mới ở vị trí ngẫu nhiên trên màn hình
    int x = SCREEN_WIDTH-gDeadline.GetWidth();  // Tọa độ x ngẫu nhiên
    int y = SCREEN_HEIGHT-gDeadline.GetHeight();  // Bắt đầu từ phía trên màn hình
    int vel=2;
...
void Check_Character_Gift_Collision(Character& character) {
    SDL_Rect characterRect = {character.Get_posx(), character.Get_posy(), gCharacter.GetWidth(), gCharacter.GetHeight()};
...
void Load_High_Score(string path) {
    ...
    if (!(file >> highscore)) {
        cerr << "Error: Unable to read high score from file " << path << endl;
        file.close();
        return;
    }
    file.close();
...
void Save_Hight_Score(string path){
    ofstream file(path);
    file<<highscore;
    file.close();
}
...
void Show_High_Score(){
    ...
    SDL_Texture *textTexture = SDL_CreateTextureFromSurface(gRenderer, textSurface);
    SDL_Rect textRect = {10, 35, textSurface->w, textSurface->h};
    SDL_RenderCopy(gRenderer, textTexture, NULL, &textRect);
    SDL_FreeSurface(textSurface);
}
void ShowScore_GameOver(){
    ...
    SDL_Texture *textTexture = SDL_CreateTextureFromSurface(gRenderer, textSurface);
    SDL_Rect textRect = {SCREEN_WIDTH/2-(textSurface->w/2),SCREEN_HEIGHT/4-(textSurface->h/2) , textSurface->w, textSurface->h};
    SDL_RenderCopy(gRenderer, textTexture, NULL, &textRect);
    SDL_FreeSurface(textSurface);
}
void Show_High_Score_Game_Over(){
    ...
    SDL_Texture *textTexture = SDL_CreateTextureFromSurface(gRenderer, textSurface);
    SDL_Rect textRect = {SCREEN_WIDTH/2-(textSurface->w/2),SCREEN_HEIGHT/4-(textSurface->h/2)+textSurface->h, textSurface->w, textSurface->h};
    SDL_RenderCopy(gRenderer, textTexture, NULL, &textRect);
    SDL_FreeSurface(textSurface);
}
...
bool loadMedia(){
    ...
    gDie=Mix_LoadWAV("music/Die.mp3");
    if (gDie==NULL)
    {
        cout << "Khong load duoc gDie trong LoadMedia Function" << Mix_GetError() << endl;
        success = false;
    }
    ///
    ///
    ...
    if( !gCharacter.LoadImage("image/ShinNew.png" ))
    {
        cout<<"Khong load duoc Character trong LoadMedia Function";
        success = false;
    }
    if( !gGameOver.LoadImage("image/GameOver.jpeg" ))
    {
...
void showMenu() {
    ...
    int back_x=SCREEN_WIDTH/2-gStartGame.GetWidth()/2;int back_x_end=toturial_x+gToturial.GetWidth();
    int back_y=SCREEN_HEIGHT/4+2*gStartGame.GetHeight()+20;int back_y_end=toturial_y+gToturial.GetHeight();
        ...
        while (SDL_PollEvent(&e) != 0) {
            if (e.type == SDL_QUIT) {
                ...
                if (x >= start_x && x <= start_x_end && y >= start_y && y <= start_y_end) {
                    ...
                    // Thoát game
                    cout << "QuitGame selected" << endl;
                    quit = true;// Thoát khỏi menu và chương trình)
                }
            }
            
            if(showMax && back==true){
